...
try{
	...
	//select * from IM_FLIGHT_ROUTES where LENGTH(flight_no)=5

		selectQuery.append("SELECT * FROM IM_FLIGHT_ROUTES where LENGTH(FLIGHT_NO)=5 and SRC_COUNTRY_CODE <>'IND' and  DEST_COUNTRY_CODE = 'IND'");
	...
	rsMain = psMain.executeQuery();  
		...
		{				
			...
			<%															
				...
				else if(flightNumber1.length() == 8)
					dep_flt_no = flightNumber1.substring(0,5) + String.valueOf(Integer.parseInt(flightNumber1.substring(5,8)) + 1);
				else if(flightNumber1.length() == 9)
					dep_flt_no = flightNumber1.substring(0,5) + String.valueOf(Integer.parseInt(flightNumber1.substring(6,9)) + 1);
				else
					dep_flt_no = "Error";
